### 以下分享如何開啟 ```anaconda prompt``` 以及創造一個虛擬環境。  
...
- ```cls```: 清除視窗。[```macOS```: ```clear```]
 ...
 #### 2. 創建一個虛擬環境(```virtual environment```):
...
- 虛擬環境的好處:
  ...
  ![so many modules](G:\我的雲端硬碟\NCCU\大二上學期(2020SEP 至 2021JAN)\數理資訊社\so many modules4.png)
...
- Install ```ipykernel```:
  ...
  4. 最後一步!
     ...
     ###### 試試看在 cell 輸入 ```import numpy```，為何出現錯誤? 該怎麼解決?
